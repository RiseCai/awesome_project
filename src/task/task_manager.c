...
void TaskGraph_AddTask(TaskGraph *graph, Task *task)
{
    if (graph->task_count == graph->capacity) {
        ...
        graph->tasks =
            (Task **)realloc(graph->tasks, new_capacity * sizeof(Task *));
        graph->capacity = new_capacity;

        // 重新分配依赖关系矩阵
        graph->dependencies =
            (int **)realloc(graph->dependencies, new_capacity * sizeof(int *));
        for (int i = 0; i < new_capacity; i++) {
            if (i >= graph->task_count) {
                graph->dependencies[i] =
                    (int *)calloc(new_capacity, sizeof(int));
            } else {
                graph->dependencies[i] = (int *)realloc(
                    graph->dependencies[i], new_capacity * sizeof(int));
...
TaskGraph *SplitOrderIntoTasks(Order *order)
{
    ...
    // 创建任务
    for (VModelPhase phase = PHASE_REQUIREMENTS; phase <= PHASE_MAINTENANCE;
         phase++) {
        Task *task = (Task *)malloc(sizeof(Task));
        InitializeTask(task, order->task_count + graph->task_count,
                       (TaskType)phase, "Placeholder task",
                       ROLE_PRODUCT_MANAGER, phase);
